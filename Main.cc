...
int calculate_total_calls(ifstream& in_file, string& line)
{
        ...
        substrings.push_back(STRING_TSPI_REPLY);
        substrings.push_back(STRING_TSPI_ASYNC);
        substrings.push_back(STRING_TSPI_EVENT);

        //Read line by line
        while(getline (in_file, line))
        {
                //Check if root string
                if(line.find(STRING_TSPI_ROOT) != string::npos)
                {
                        //This is potentially a TAPI call 
